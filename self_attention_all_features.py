...
class TransModel(nn.Module):
    ...
    def forward(self, seqs_info, type_info, seq_ls, targets=None, weights=None):
        ...
        if targets is not None:
            ...
            tp_w = torch.tensor([0.]).to(device)
            ex_weights = torch.tensor([1.]*100).to(device)
            for i in range(len(seq_ls)):
                tp_l = torch.cat((tp_l, logits[i][:seq_ls[i]]))
                tp_t = torch.cat((tp_t, targets[i][:seq_ls[i]]))
                if weights is not None:
                    tp_w = torch.cat((tp_w, weights[i][:seq_ls[i]]))
                else:
                    tp_w = torch.cat((tp_w, ex_weights[:seq_ls[i]]))
                
            tp_l = tp_l[1:]
            tp_t = tp_t[1:]
            tp_w = tp_w[1:]
            loss = self.cri(tp_l, tp_t)
            loss = loss * tp_w
            pred = torch.argmax(tp_l, dim=1)
            acc = sum((pred==tp_t)*tp_w)/sum(tp_w)
            
        return logits, loss.mean(), acc
